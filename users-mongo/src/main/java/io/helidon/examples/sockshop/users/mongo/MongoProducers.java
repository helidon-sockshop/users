...
public class MongoProducers {
     ...
     */
    public static final String DEFAULT_HOST = "users-db";
    ...
    public static MongoClient client(Config config) {
        Config users = config.get("db");
        return client(users.get("host").asString().orElse(DEFAULT_HOST),
                users.get("port").asInt().orElse(DEFAULT_PORT));
    }
    ...
    /**
     * CDI Producer for the {@code MongoCollection} that contains users.
     *
     * @param db the MongoDB database to use
     *
     * @return a {@code MongoCollection} instance for the users
     */
    @Produces
    @ApplicationScoped
    public static MongoCollection<User> users(MongoDatabase db) {
